  ...
  useEffect(() => {
    try {
      ...
      if (savedDocs.length > 0) {
        ...
        // Filtrer pour ne garder que les documents Supabase
        const supabaseDocs = savedDocs.filter(item => 
          item.document.url && 
          typeof item.document.url === 'string' && 
          item.document.url.includes('supabase.co')
        );
        
        console.log('Documents Supabase trouvés:', supabaseDocs.length);
        
        // Ajouter uniquement les documents Supabase
        supabaseDocs.forEach(item => {
        ...
        // Mettre à jour les données avec uniquement les documents Supabase
        setReportsData(newData);
        
        // Mettre à jour localStorage pour ne conserver que les documents Supabase
        localStorage.setItem('uploaded_documents', JSON.stringify(supabaseDocs));
      }
      
      // Marquer comme initialisé pour les futures sessions
      localStorage.setItem('reports_initialized', 'true');
    } catch (error) {
