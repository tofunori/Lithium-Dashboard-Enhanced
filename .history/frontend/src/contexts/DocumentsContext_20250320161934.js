...
export const DocumentsProvider = ({ children }) => {
  ...
  const addDocument = async (document, file, fileUrl) => {
    try {
      ...
      // Si un supabasePath est fourni, c'est probablement un PDF stocké dans Supabase
      if (document.supabasePath && document.supabasePath.includes('.pdf')) {
        documentFormat = 'pdf';
        console.log("Format défini sur PDF car le document a un chemin Supabase PDF");
      }
      
      // Si l'URL se termine par .pdf, c'est aussi un PDF
