...
// Configurer le worker pour pdfjs
...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const extractPdfMetadata = async (file) => {
    ...
    try {
      ...
      // Traiter le nom du fichier pour extraire des informations supplémentaires
      const fileName = file.name.replace('.pdf', '');
      const extractedTitle = metadata.info?.Title || fileName
        .replace(/[-_]/g, ' ')
      ...
      setSnackbar({
        open: true,
        message: 'Impossible d\'extraire toutes les métadonnées. Certains champs ont été pré-remplis.',
        severity: 'warning'
      });
    }
  };
