...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Téléversement en cours vers ImgBB...',
        severity: 'info'
      });
      ...
      // Créer un objet FormData pour l'API ImgBB
      const formData = new FormData();
      formData.append('key', 'bc49fad4954caafbf39393de7a11cb31'); // Clé API ImgBB
      formData.append('image', base64File.split(',')[1]); // Supprimer le préfixe "data:image/..."
      
      // Envoyer la requête à l'API ImgBB en utilisant fetch au lieu d'axios
      ...
      if (data.success) {
        ...
        // Créer les données du document avec l'URL ImgBB
        const documentWithURL = {
          ...documentData,
          url: imageUrl,
          // Ajouter les métadonnées supplémentaires
          uploadDate: new Date().toISOString(),
          externalStorage: 'imgbb',
          deleteUrl: data.data.delete_url || ''
        };
        
        // Ajouter le document à la collection via le contexte
        const addedDocument = addDocument(documentWithURL, selectedFile, imageUrl);
        
        // Notification de succès
        setSnackbar({
          open: true,
          message: 'Document téléversé avec succès vers ImgBB!',
          severity: 'success'
        });
        ...
        throw new Error('Échec du téléversement vers ImgBB');
      }
    } catch (error) {
      console.error('Erreur lors de l\'ajout du document:', error);
      
      // En cas d'échec, revenir à la méthode locale
      fallbackToLocalStorage(error);
    }
  };
  
  // Fonction de secours pour le stockage local
  const fallbackToLocalStorage = (error) => {
    try {
      console.warn('Utilisation du stockage local comme solution de secours');
      ...
      setSnackbar({
        open: true,
        message: 'Document ajouté en mode local (échec du téléversement en ligne).',
        severity: 'warning'
      });
      ...
      console.error('Erreur lors de la solution de secours:', secondError);
      setSnackbar({
        open: true,
