...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let documentUrl = '';
      let isPermanentStorage = false;
      
      try {
        // Essayer d'uploader vers Firebase
        // Créer une référence Firebase Storage avec un nom unique
        ...
        console.log('Fichier téléversé avec succès!', uploadResult);
        ...
        console.error('Erreur Firebase, utilisation du mode local:', firebaseError);
        // En cas d'erreur, utiliser le mode local
        documentUrl = URL.createObjectURL(selectedFile);
        isPermanentStorage = false;
        
        // Avertir l'utilisateur que le stockage est temporaire
        setSnackbar({
          open: true,
          message: 'Document ajouté en mode local (temporaire). Il disparaîtra après fermeture du navigateur.',
          severity: 'warning'
        });
      }
      ...
      const addedDocument = addDocument(documentWithURL, selectedFile, documentUrl);
      
      // Notification de succès
      setSnackbar({
        open: true,
        message: isPermanentStorage 
          ? 'Document téléversé avec succès et sauvegardé de façon permanente!' 
          : 'Document ajouté temporairement (mode local).',
        severity: isPermanentStorage ? 'success' : 'warning'
      });
