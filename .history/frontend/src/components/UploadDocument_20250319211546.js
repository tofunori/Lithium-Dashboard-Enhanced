...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Solution simple: utiliser le mode local uniquement, mais avec persistence
      const localUrl = URL.createObjectURL(selectedFile);
      
      // Créer un nom de fichier persistant (seulement pour référence)
      ...
      const newDocument = {
        ...documentData,
        url: localUrl,
        fileName: persistentFileName, // Stocker le nom pour référence
        isPermanentStorage: true,     // Important pour la persistance
        uploadDate: new Date().toISOString()
      };
      
      // Ajouter le document au contexte
      const addedDocument = addDocument(newDocument, selectedFile, localUrl);
      
      // Notification de succès
      setSnackbar({
        open: true,
        message: 'Document ajouté avec succès en mode local!',
        severity: 'success'
      });
