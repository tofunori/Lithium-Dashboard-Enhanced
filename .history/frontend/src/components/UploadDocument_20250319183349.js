...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Config Supabase:", supabase);
      
      // Vérifier les buckets disponibles
      const { data: buckets, error: bucketsError } = await supabase.storage.listBuckets();
      console.log("Buckets disponibles:", buckets, "Erreur:", bucketsError);
      
      // Vérifier si le bucket 'documents' existe, sinon essayer de le créer
      ...
      if (!bucketExists) {
        ...
        try {
          ...
          console.log("Résultat création bucket:", data, "Erreur:", error);
        } catch (bucketError) {
      ...
      const fileName = `${Date.now()}_${documentData.title.replace(/\s+/g, '_')}.${fileExt}`;
      const filePath = `${documentData.foundryId || 'general'}/${fileName}`;
      ...
      console.log("Résultat upload:", data, "Erreur:", error);
      
      if (error) {
        // Si l'erreur concerne les permissions ou le bucket
        if (error.message.includes('bucket') || error.message.includes('permission')) {
          throw new Error(`Problème d'accès au stockage Supabase: ${error.message}. Assurez-vous que le bucket 'documents' existe et que les permissions sont configurées.`);
        }
        throw error;
      }
      ...
      if (data) {
        // Upload réussi, obtenir l'URL Supabase
        ...
        // En cas d'échec, utiliser une URL temporaire locale
        console.warn("Utilisation d'une URL temporaire locale au lieu de Supabase");
        publicUrl = URL.createObjectURL(selectedFile);
      }
      
      // Créer les données du document avec l'URL
      ...
      const addedDocument = addDocument(documentWithURL, selectedFile, publicUrl);
