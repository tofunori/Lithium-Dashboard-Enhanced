  ...
  return (
    <>
      {validPlants.map((plant) => {
        ...
        // Utiliser une formule plus agressive qui amplifie les différences
        const sizeFactor = maxProduction > 0 
          ? 0.8 + (Math.pow(production / maxProduction, 0.6) * 3.0)  // Utiliser un exposant fractionnaire pour mieux répartir les valeurs
          : 1;
