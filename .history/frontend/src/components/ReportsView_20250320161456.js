...
const ReportsView = ({ foundryId }) => {
  ...
  const extractWebPageInfo = async (url) => {
    try {
      ...
      // Vérifier si c'est un PDF ou un autre fichier
      const isPdf = url.toLowerCase().endsWith('.pdf');
      const isFile = /\.(pdf|doc|docx|ppt|pptx|xls|xlsx)$/i.test(url);
      
      if (isPdf || isFile) {
        // Extraire le nom du fichier depuis l'URL
        const fileName = url.split('/').pop();
        const fileNameWithoutExt = fileName.replace(/\.[^/.]+$/, "");
        // Convertir format du nom de fichier en titre plus lisible
        ...
        // Si c'est un PDF de Redwood, ajouter une description spécifique
        let description = `Fichier ${isPdf ? 'PDF' : 'document'} externe`;
        if (url.includes('Redwood')) {
          description = "Présentation sur les activités de recyclage de Redwood Materials, entreprise spécialisée dans le recyclage de batteries lithium-ion.";
        }
        
        setSnackbar({
          open: true,
          message: isPdf ? 'PDF détecté. Le fichier sera téléchargé et stocké localement après validation.' : 'Informations extraites depuis le nom du fichier',
          severity: 'success'
        });
        
        return { 
          title: title || 'Document externe', 
          description 
        };
      }
      
      // Pour les pages web normales, utiliser le proxy CORS
      ...
      // Si on arrive ici, on n'a pas pu extraire les infos - essayer avec le nom de domaine
      const domain = new URL(url).hostname.replace('www.', '');
      return { 
        title: `Document depuis ${domain}`, 
        description: `Ressource externe provenant de ${domain}` 
      };
    } catch (error) {
      console.error('Erreur lors de l\'extraction des informations:', error);
      
      try {
        // Dernière tentative : extraire juste le nom du fichier/domaine de l'URL
        const urlParts = url.split('/');
        const lastPart = urlParts[urlParts.length - 1];
        const title = lastPart.split('.')[0].replace(/[-_]/g, ' ');
        
        setSnackbar({
          open: true,
          message: 'Extraction limitée aux informations de base',
          severity: 'warning'
        });
        
        return { 
          title: title || 'Document externe',
          description: 'Ressource externe'
        };
      } catch (e) {
        setSnackbar({
          open: true,
