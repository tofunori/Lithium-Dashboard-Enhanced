...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Téléversement en cours vers Firebase...',
        severity: 'info'
      });
      
      // Créer un nom de fichier persistant
      const timestamp = Date.now();
      const fileName = selectedFile.name.replace(/[^a-zA-Z0-9.-]/g, '_');
      const persistentFileName = `document_${timestamp}_${fileName}`;
      
      // Téléverser le fichier vers Firebase Storage
      const storageRef = ref(storage, `documents/${persistentFileName}`);
      ...
      const newDocument = {
        ...
        firebasePath: `documents/${persistentFileName}` // Stocker le chemin Firebase pour suppression future
      };
      
      // Ajouter le document au contexte
      const addedDocument = addDocument(newDocument, selectedFile, firebaseUrl);
      
      // Notification de succès
      setSnackbar({
        open: true,
        message: 'Document téléversé avec succès vers Firebase!',
        severity: 'success'
      });
      ...
      console.error('Erreur lors du téléversement vers Firebase:', error);
      setSnackbar({
        open: true,
        message: 'Une erreur est survenue lors du téléversement: ' + error.message,
        severity: 'error'
      });
    }
  };
