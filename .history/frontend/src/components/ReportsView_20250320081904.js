...
const ReportsView = ({ foundryId }) => {
  ...
  useEffect(() => {
    // Ne recharger que si :
    // - Aucun chargement n'est en cours
    // - Il n'y a pas d'erreur de chargement actuelle
    // - La liste des rapports est vide
    if (!isLoading && !loadError && reports.length === 0) {
      console.log("Pas de documents trouvés, chargement initial...");
      loadPublicDocuments();
    }
  }, [reports.length, isLoading, loadPublicDocuments, loadError]);
