...
const MapLibreView = ({ plants = [], onResize }) => {
  ...
  useEffect(() => {
    ...
    // Activer également le zoom à la molette
    ...
    map.on('load', () => {
      console.log('Carte chargée - ajout des sources et couches');
  ...
  }, [mapStyle, t, markersData, viewState]); // Ajouter markersData et viewState dans les dépendances
  
  // Effet pour mettre à jour les données des points sur la carte
  useEffect(() => {
    console.log('Mise à jour des données des points', markersData);
    const map = mapInstance.current;
    if (!map || !map.loaded()) return;
    
    // Vérifier si la source existe avant de mettre à jour
    if (map.getSource('plants-data')) {
      console.log('Mise à jour des données existantes', markersData.features.length, 'points');
