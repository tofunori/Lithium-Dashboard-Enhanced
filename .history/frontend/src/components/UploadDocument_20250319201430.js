...
const UploadDocument = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Utiliser directement le mode local à cause des erreurs CORS avec Firebase
      const localUrl = URL.createObjectURL(selectedFile);
      console.log("URL locale créée:", localUrl);
      
      // Ajouter le document en mode local mais avec stockage permanent
      const localDocument = {
        ...documentData,
        url: localUrl,
        isPermanentStorage: true,  // Marquer comme permanent pour qu'il soit sauvegardé
        uploadDate: new Date().toISOString()
      };
      
      const addedDocument = addDocument(localDocument, selectedFile, localUrl);
      
      // Notification pour le mode local
      setSnackbar({
        open: true,
        message: 'Document ajouté avec succès! (Mode local avec persistance)',
        severity: 'success'
      });
