...
const ReportsView = ({ foundryId }) => {
  ...
  const downloadAndStorePdf = async (pdfUrl, title) => {
    try {
      ...
      // Utiliser un proxy CORS si nécessaire pour éviter les problèmes de CORS
      const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(pdfUrl)}`;
      
      // Étape 1: Récupérer le contenu du PDF en essayant d'abord l'URL directe, puis via le proxy si nécessaire
      let response;
      try {
        // Essayer d'abord l'URL directe
        response = await fetch(pdfUrl, { mode: 'cors' });
        console.log("Réponse directe obtenue:", response.status);
      } catch (directError) {
        // Si l'erreur est liée au CORS, essayer via le proxy
        console.log("Erreur accès direct, utilisation du proxy:", directError);
      ...
      console.log("Blob PDF récupéré:", pdfBlob.type, pdfBlob.size);
      
      // Vérifier que c'est bien un PDF ou que le blob a une taille raisonnable
      if ((pdfBlob.type !== 'application/pdf' && !pdfUrl.toLowerCase().endsWith('.pdf')) && pdfBlob.size < 5000) {
      ...
      setSnackbar({
        open: true,
        message: 'PDF stocké avec succès!',
        severity: 'success'
      });
