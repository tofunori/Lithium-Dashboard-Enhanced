...
const InstallationsView = () => {
  ...
  // Charger les données de refineryData au démarrage
  useEffect(() => {
    if (refineryData && refineryData.refineries) {
      setRefineries(refineryData.refineries);
    }
  }, []);
  ...
  const handleSaveRefinery = () => {
    ...
    let updatedRefineries;
    
    if (editMode) {
      // Modification d'une raffinerie existante
      updatedRefineries = refineries.map(refinery => 
        refinery.id === formData.id ? formData : refinery
      );
      ...
      updatedRefineries = [...refineries, formData];
    ...
    // Mise à jour du fichier de données (simulation)
    console.log('Données mises à jour:', updatedRefineries);
    
    // Dans un environnement réel, on enverrait les données au backend
