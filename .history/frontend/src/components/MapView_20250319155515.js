              ...
              <Box sx={{ minWidth: 240, maxWidth: 300 }}>
                ...
                {plant.website && (
                  <Typography variant="body2" sx={{ mt: 1 }}>
                    ...
                    <Link href={plant.website} target="_blank" rel="noopener noreferrer" sx={{ ml: 0.5 }}>
                      {t('visit')}
                    </Link>
                  </Typography>
                )}
...
const MapView = ({ plants, onResize }) => {
  ...
  const mapContainerStyle = {
    ...
    width: '100%',
    maxWidth: `${mapHeight}px`, // Pour le rendre carré
    ...
    minWidth: '300px',
    aspectRatio: '1/1' // Forcer un rapport d'aspect carré
  };
  ...
  return (
    <Paper 
      ...
      sx={{ 
        ...
        transition: settings.animations ? 'height 0.3s ease-in-out' : 'none',
        width: '100%',
        maxWidth: `${mapHeight + 40}px`, // +40px pour tenir compte du padding
        margin: '0 auto',
        display: 'flex',
    ...
    >
      ...
      {/* Légende mise à jour */}
      {settings.showLegend && renderLegend()}
    </Paper>
  );
};
